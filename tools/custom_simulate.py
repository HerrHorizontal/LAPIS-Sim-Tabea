...
def ini_and_run(job_file, pool_files, storage_file, storage_type,
                log_file="test.log",  remote_throughput=1., seed=1234,
                until=None, calculation_efficiency=1.0, log_telegraf=False):
    logging.getLogger("implementation").info(job_file, pool_files, storage_file, log_file)
    # ini logging to file
...
ini_and_run(job_file=sys.argv[1], pool_files=[sys.argv[2], sys.argv[3]],
            storage_file=sys.argv[4], storage_type="filehitrate", log_file=sys.argv[5],
            remote_throughput=sys.argv[6], calculation_efficiency=sys.argv[7],
            log_telegraf=False)